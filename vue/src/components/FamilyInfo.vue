  ...
  <section>
    <table>
      <tr id="fullName">
        <td>Full Name:</td>
        <td>
          <span v-show="!showFull && !pending.fullName" class="data">{{
            this.family.fullName
          }}</span>
          <span v-show="!showFull && pending.fullName" class="newValue">{{
        ...
        <td class="editButtons">
          <button
            ...
            v-on:click.prevent="
              showFull = true;
              newData.fullName = family.fullName;
            "
            v-show="!showFull"
            v-bind:disabled="pending.fullName"
            v-if="!($store.state.user.role == 'admin' && pending.fullName)"
          >
            Edit
          </button>
          <button
            type="button"
            v-on:click="rejectRequest('fullName')"
            v-if="$store.state.user.role == 'admin' && pending.fullName"
          >
            Reject
          </button>
          <button
            type="submit"
            v-on:click="saveChange('fullName')"
            v-show="showFull"
            v-bind:disabled="!newData.fullName"
          >
            Save
          </button>
          <button
      ...
      <tr id="city">
        <td>City:</td>
        <td>
          <span v-show="!showCity && !pending.city" class="data">{{
            this.family.city
          }}</span>
          <span v-show="!showCity && pending.city" class="newValue">{{
            this.pending.city
          }}</span>
        ...
        <td class="editButtons">
          <button
            ...
            v-on:click.prevent="
              showCity = true;
              newData.city = family.city;
            "
            v-show="!showCity"
            v-bind:disabled="pending.city"
            v-if="!($store.state.user.role == 'admin' && pending.city)"
          >
            Edit
          </button>
          <button
            type="button"
            v-on:click="rejectRequest('city')"
            v-if="$store.state.user.role == 'admin' && pending.city"
          >
            Reject
          </button>
          <button
            type="submit"
            v-on:click="saveChange('city')"
            v-show="showCity"
            v-bind:disabled="!newData.city"
          >
            Save
          </button>
          <button
      ...
      <tr id="state">
        <td>State:</td>
        <td>
          <span v-show="!showState && !pending.state" class="data">{{
            this.family.state
          }}</span>
          <span v-show="!showState && pending.state" class="newValue">{{
            this.pending.state
          }}</span>
        ...
        <td class="editButtons">
          <button
            ...
            v-on:click.prevent="
              showState = true;
              newData.state = family.state;
            "
            v-show="!showState"
            v-bind:disabled="pending.state"
            v-if="!($store.state.user.role == 'admin' && pending.state)"
          >
            Edit
          </button>
          <button
            type="button"
            v-on:click="rejectRequest('state')"
            v-if="$store.state.user.role == 'admin' && pending.state"
          >
            Reject
          </button>
          <button
            type="submit"
            v-on:click="saveChange('state')"
            v-show="showState"
            v-bind:disabled="!newData.state"
          >
            Save
          </button>
          <button
      ...
      <tr id="zip">
        <td>Zip:</td>
        <td>
          <span v-show="!showZip && !pending.zip" class="data">{{
            this.family.zip
          }}</span>
          <span v-show="!showZip && pending.zip" class="newValue">{{
            this.pending.zip
          }}</span>
        ...
        <td class="editButtons">
          <button
            ...
            v-on:click.prevent="
              showZip = true;
              newData.zip = family.zip;
            "
            v-show="!showZip"
            v-bind:disabled="pending.zip"
            v-if="!($store.state.user.role == 'admin' && pending.zip)"
          >
            Edit
          </button>
          <button
            type="button"
            v-on:click="rejectRequest('zip')"
            v-if="$store.state.user.role == 'admin' && pending.zip"
          >
            Reject
          </button>
          <button
            type="submit"
            v-on:click="saveChange('zip')"
            v-show="showZip"
            v-bind:disabled="!newData.zip"
          >
            Save
          </button>
          <button
      ...
      <tr id="phoneNumber">
        <td>Phone Number:</td>
        <td>
          <span v-show="!showPhone && !pending.phoneNumber" class="data">{{
            this.family.phoneNumber
          }}</span>
          <span v-show="!showPhone && pending.phoneNumber" class="newValue">{{
            this.pending.phoneNumber
          }}</span>
        ...
        <td class="editButtons">
          <button
            ...
            v-on:click.prevent="
              showPhone = true;
              newData.phoneNumber = family.phoneNumber;
            "
            v-show="!showPhone"
            v-bind:disabled="pending.phoneNumber"
            v-if="!($store.state.user.role == 'admin' && pending.phoneNumber)"
          >
            Edit
          </button>
          <button
            type="button"
            v-on:click="rejectRequest('phone_number')"
            v-if="$store.state.user.role == 'admin' && pending.phoneNumber"
          >
            Reject
          </button>
          <button
            type="submit"
            v-on:click="saveChange('phone_number')"
            v-show="showPhone"
            v-bind:disabled="!newData.phoneNumber"
          >
            Save
          </button>
          <button
      ...
      <tr id="emailAddress">
        <td>Email:</td>
        <td>
          <span v-show="!showEmail && !pending.emailAddress" class="data">{{
            this.family.emailAddress
          }}</span>
          <span v-show="!showEmail && pending.emailAddress" class="newValue">{{
            this.pending.emailAddress
          }}</span>
          <input
            type="text"
            v-model.number="newData.emailAddress"
            v-show="showEmail"
          />
        </td>
        <td class="editButtons">
          <button
            ...
            v-on:click.prevent="
              showEmail = true;
              newData.emailAddress = family.emailAddress;
            "
            v-show="!showEmail"
            v-bind:disabled="pending.emailAddress"
            v-if="!($store.state.user.role == 'admin' && pending.emailAddress)"
          >
            Edit
          </button>
          <button
            type="button"
            v-on:click="rejectRequest('email_address')"
            v-if="$store.state.user.role == 'admin' && pending.emailAddress"
          >
            Reject
          </button>
          <button
            type="submit"
            v-on:click="saveChange('email_address')"
            v-show="showEmail"
            v-bind:disabled="!newData.emailAddress"
          >
            Save
          </button>
          <button
      ...
      <tr id="campers">
        ...
        <td>
          <div class="data">
            <ul>
              <li
                v-for="camper in family.campers"
              ...
              >
                <router-link
                  ...
                  >{{ camper.firstName }} {{ camper.lastName }}</router-link
                >
              </li>
            </ul>
          </div>
